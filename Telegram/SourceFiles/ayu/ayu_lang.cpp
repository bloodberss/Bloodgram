// This is the source code of Bloodgram for Desktop.
//
...
void AyuLanguage::loadCachedLanguage() {
	...
	if (file.open(QIODevice::ReadOnly)) {
		...
		if (error.error == QJsonParseError::NoError) {
			LOG(("Loading cached Bloodgram language: %1").arg(finalLangPackId));
...
void AyuLanguage::saveCachedLanguage(const QByteArray &json, const QString &langId) {
	...
	if (file.open(QIODevice::WriteOnly)) {
		file.write(json);
		file.close();
		LOG(("Cached Bloodgram language: %1").arg(langId));
	}
}
...
void AyuLanguage::fetchLanguage(const QString &id, const QString &baseId) {
	...
	// https://crowdin.com/project/bloodgram/discussions/6
	QUrl url;
	if (!finalLangPackId.isEmpty() && !baseId.isEmpty() && !needFallback) {
		url.setUrl(qsl("https://cdn.jsdelivr.net/gh/Bloodgram/Languages@l10n_main/values/langs/%1/Shared.json").arg(
			finalLangPackId));
	} else {
		url.setUrl(qsl("https://cdn.jsdelivr.net/gh/Bloodgram/Languages@l10n_main/values/langs/%1/Shared.json").arg(
...
void AyuLanguage::fetchFinished() {
	...
	if (statusCode == 404 && !langPackId.isEmpty() && !langPackBaseId.isEmpty() && !needFallback) {
		LOG(("Bloodgram Language not found! Fallback to main language: %1...").arg(langPackBaseId));
		needFallback = true;
...
void AyuLanguage::fetchError(QNetworkReply::NetworkError e) {
	...
	if (e == QNetworkReply::NetworkError::ContentNotFoundError) {
		...
		if (!id.isEmpty() && !baseId.isEmpty() && !needFallback) {
			LOG(("Bloodgram Language not found! Fallback to main language: %1...").arg(baseId));
			needFallback = true;
			_chkReply->disconnect();
			fetchLanguage("", baseId);
		} else {
			LOG(("Bloodgram Language not found!"));
			_chkReply = nullptr;
		}
	}
}
