...
void BuildCategories(SectionBuilder &builder) {
	...
	builder.addSectionButton({
		.title = rpl::single(QString("Bloodgram")),
...
void BuildLinks(SectionBuilder &builder) {
	...
	builder.addButton({
		...
		.label = rpl::single(QString("@bloodgram")),
		.onClick = [=] {
			controller->showPeerByLink(Window::PeerByLinkInfo{
				.usernameOrId = QString("bloodgram"),
			});
		},
	});
	builder.addButton({
		...
		.label = rpl::single(QString("@bloodgramchat")),
		.onClick = [=] {
			controller->showPeerByLink(Window::PeerByLinkInfo{
				.usernameOrId = QString("bloodgramchat"),
			});
		},
	});
	builder.addButton({
		...
		.onClick = [=] {
			QDesktopServices::openUrl(
				QString("https://translate.bloodgram.one"));
		},
	});
	builder.addButton({
		.id = u"ayu/website"_q,
		...
		.label = rpl::single(QString("docs.bloodgram.one")),
		.onClick = [=] {
			QDesktopServices::openUrl(
				QString("https://docs.bloodgram.one"));
		},
	});
