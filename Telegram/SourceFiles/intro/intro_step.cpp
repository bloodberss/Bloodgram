...
void Step::paintCover(QPainter &p, int top) {
	...
	const auto bloodgramIcon = Ui::PixmapFromImage(AyuAssets::currentAppLogo());
	QIcon(bloodgramIcon).paint(&p, QRect(width() / 2 - bloodgramIcon.width() / 2, planeTop - 16, bloodgramIcon.width(), st::introCoverIcon.height()));
}
