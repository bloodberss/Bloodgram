...
rpl::producer<TextWithEntities> Text() {
	return tr::lng_about_text2(
		...
		rpl::single(Ui::Text::Link(
			"GNU GPL",
			"https://github.com/Bloodgram/BloodgramDesktop/blob/dev/LICENSE")),
		lt_github_link,
		rpl::single(Ui::Text::Link(
			"GitHub",
			"https://github.com/Bloodgram/BloodgramDesktop")),
		tr::marked);
}
...
void AboutBox(not_null<Ui::GenericBox*> box, Window::SessionController* controller) {
	box->setTitle(rpl::single(u"Bloodgram Desktop"_q));
	...
	box->addLeftButton(
		rpl::single(QString("@BloodgramReleases")),
		[box, controller]
		{
			box->closeBox();
			controller->showPeerByLink(Window::PeerByLinkInfo{
				.usernameOrId = QString("bloodgramreleases"),
			});
		});
